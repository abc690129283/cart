<template>
  <div class="cart">
    ...
    <!--productDictList[item.id]指向当前列表选中的数据-->
    <div class="cart-content">
      <!--[item.id]指向列表选中的id productDictList指向字典-->
      <div class="cart-content-main" v-for="(item,index) in cartList">
        ...
        <div class="cart-delete">
          <span class="cart-control-delate" @click="handleDelete(index)">删除</span>
        </div>
      </div>
...
export default {
  ...
  methods: {
    handleCount(index, count) {
        //  点击判断 1||-1 小于0 与数量等于1 则返回
      if (count < 0 && this.cartList[index].count === 1) return;
      // 执行mutation里面的函数 并且传参数
      this.$store.commit('editCartCount',{
          id:this.cartList[index].id, // 当前点击的id
          count:count
      })
    },
    handleDelete(index){
    ...
    // 点击购买 把购物车清空之后  也就是更新了buy 出现弹窗
    handleOrder(){
        this.$store.dispatch('buy').then(()=>{  // then==》执行成功后
            window.alert('购买成功!');
        })
    }
  }
